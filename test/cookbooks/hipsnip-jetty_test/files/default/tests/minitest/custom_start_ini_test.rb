...
describe_recipe "hipsnip-jetty_test::default" do
  ...
  it "should contain a customized start.ini file with the appropriate values" do
    path = File.join(node['jetty']['home'],'start.ini')
    start_ini_file_exists = File.exists?(path)
    assert_equal(true,start_ini_file_exists,'start.ini file exists')
    File.open(path,'r') do |file|
      content = file.read
      node['jetty']['start_ini']['content'].each do |option|
        assert_equal(true,!!content.match(Regexp.new "^#{option}$"),"instruction #{option}")
      end
    end
  end
end